...
public class GradleScriptVisitor extends CodeVisitorSupport {
		...
		if (expressions != null) {
			...
			if ((expressions.size() == 1) && (expressions.get(0) instanceof ConstantExpression)) {
				...
				if (constantExpression != null) {
					...
					if (expressionText != null && inPlugins && !inBuildScript && !inAllProjects && !inPluginManagement
							&& !inDependencyResolutionManagement) {
						plugins.add(new Plugin(expressionText));
					}

					if (expressionText != null && repositoriesConfigurationName != null && inRepositories
							&& !inBuildScript && !inAllProjects && !inPluginManagement
							&& !inDependencyResolutionManagement && !inFlatDir) {
						repositories.add(new Repository(repositoriesConfigurationName, expressionText));
					}

					if (expressionText != null && repositoriesConfigurationName != null && inBuildScriptRepositories
							&& !inAllProjectsRepositories && !inPluginManagementRepositories
							&& !inDependencyResolutionManagementRepositories && !inFlatDir) {

						buildscriptRepositories.add(new Repository(repositoriesConfigurationName, expressionText));
					}

					if (expressionText != null && repositoriesConfigurationName != null && inAllProjectsRepositories
							&& !inBuildScriptRepositories && !inPluginManagementRepositories
							&& !inDependencyResolutionManagementRepositories && !inFlatDir) {
						allprojectsRepositories.add(new Repository(repositoriesConfigurationName, expressionText));
					}

					if (expressionText != null && repositoriesConfigurationName != null
							&& inPluginManagementRepositories && !inAllProjectsRepositories
							&& !inBuildScriptRepositories && !inDependencyResolutionManagementRepositories
							&& !inFlatDir) {

						pluginManagementRepositories.add(new Repository(repositoriesConfigurationName, expressionText));
					}

					if (expressionText != null && repositoriesConfigurationName != null
							&& inDependencyResolutionManagementRepositories && !inBuildScriptRepositories
							&& !inPluginManagementRepositories && !inAllProjectsRepositories && !inFlatDir) {
						dependencyResolutionManagementRepositories
								.add(new Repository(repositoriesConfigurationName, expressionText));
					}

					if (expressionText != null && dependenciesConfigurationName != null
	...
	public void visitMethodCallExpression(MethodCallExpression methodCallExpression) {
		...
		if (inDependencyResolutionManagement && inRepositories && inFlatDir && inFlatDirDirs) {
			dependencyresolutionmanagementrepositoriesflatDirdirsLastLineNumber = methodCallExpression
					.getLastLineNumber();
		...
		if (inRepositories && !inBuildScript && !inAllProjects && !inPluginManagement
				&& !inDependencyResolutionManagement && !inFlatDir) {
		...
		if (inBuildScriptRepositories && !inFlatDir) {
			...
			if (methodName.equals("mavenCentral")) {
				buildscriptRepositories.add(new Repository(methodName, "https://repo.maven.apache.org/maven2/"));
			}
			if (methodName.equals("gradlePluginPortal")) {
		...
		if (inAllProjectsRepositories && !inFlatDir) {
			...
			if (methodName.equals("mavenCentral")) {
				allprojectsRepositories.add(new Repository(methodName, "https://repo.maven.apache.org/maven2/"));
			}
			if (methodName.equals("gradlePluginPortal")) {
		...
		if (inAllProjectsRepositories && inAllProjectsRepositoriesFlatDirDirs
		...
		if (inPluginManagementRepositories && !inFlatDir) {
			...
			if (methodName.equals("mavenCentral")) {
				pluginManagementRepositories.add(new Repository(methodName, "https://repo.maven.apache.org/maven2/"));
			}
			if (methodName.equals("gradlePluginPortal")) {
				pluginManagementRepositories.add(new Repository(methodName, "https://plugins.gradle.org/m2/"));
			}
		}
		...
		if (inDependencyResolutionManagementRepositories && !inFlatDir) {
			...
			if (methodName.equals("mavenCentral")) {
				dependencyResolutionManagementRepositories
						.add(new Repository(methodName, "https://repo.maven.apache.org/maven2/"));
			}
			if (methodName.equals("gradlePluginPortal")) {
				dependencyResolutionManagementRepositories
						.add(new Repository(methodName, "https://plugins.gradle.org/m2/"));
			}
		}
							...
							if (constantExpression != null) {
								...
								if (constantExpressionText != null) {
									dependencyresolutionmanagementrepositoriesflatDirDirs
											.add(new FlatDir(constantExpressionText));
								}
							}
						}
					}
		...
		} else if (inRepositories && (blockStatementStack.isEmpty() ? false : blockStatementStack.peek())) {
			repositoriesConfigurationName = methodName;
			super.visitMethodCallExpression(methodCallExpression);
			repositoriesConfigurationName = null;

		} else if (inBuildScriptRepositories && !(blockStatementStack.isEmpty() ? false : blockStatementStack.peek())) {

			buildscriptRepositoriesConfigurationName = methodName;
			super.visitMethodCallExpression(methodCallExpression);
			buildscriptRepositoriesConfigurationName = null;

		} else if (inAllProjectsRepositories && !(blockStatementStack.isEmpty() ? false : blockStatementStack.peek())) {
		...
		} else if (inDependencies && (blockStatementStack.isEmpty() ? false : blockStatementStack.peek())) {
			dependenciesConfigurationName = methodName;
			super.visitMethodCallExpression(methodCallExpression);
			dependenciesConfigurationName = null;

		} else if (inBuildScriptDependencies && !(blockStatementStack.isEmpty() ? false : blockStatementStack.peek())) {
